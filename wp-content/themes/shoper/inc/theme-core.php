	...
	function shoper_setup() {
		...
		*/
		...
		if ( is_customize_preview() ) {
			...
			add_theme_support( 'starter-content', shoper_get_starter_content() );
		}
...
function shoper_widgets_init() {
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Homepage Banner', 'shoper' ),
		'id'            => 'slider',
		'description'   => esc_html__( 'Add widgets here.', 'shoper' ),
...
function shoper_scripts() {
	...
	wp_enqueue_script( 'shoper-js', get_theme_file_uri( '/assets/js/shoper.js'), array('jquery','thickbox' ), '1.0.0', true);
...
function shoper_register_block_pattern_categories()
{
	register_block_pattern_category(
		'shoper',
		array(
			'label'       => __('Shoper', 'shoper'),
		)
	);
}
add_action('init', 'shoper_register_block_pattern_categories');